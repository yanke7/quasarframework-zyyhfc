    ...
    >
      ...
      <template #column-body="{ column }">
        <template v-if="column.id === 'over-due'">
          ...
          <div
            class="plannerpage-column"
            data-column="overdue"
            @dragover.stop="onDragOver"
            @drop.stop="onDrop"
          >
            <transition-group name="plannerpage-item">
...
export default {
  ...
  methods: {
    ...
    onDragEnter(e, column, child) {
      if (!column && !child) {
        column = this.getCorrectTarget(e.currentTarget, 'plannerpage-column');
    ...
    findTargets() {
      ...
      for (let i = 0; i < els.length; ++i) {
        ...
        if (el.classList.contains('planner-item')) {
          child = el;
        } else if (el.classList.contains('plannerpage-column')) {
          column = el;
        }
...
.plannerpage-column.drag-over
  border: 1px dashed red

.plannerpage-item.drag-over-item
  background: rgba(0,255,0, .1)

.plannerpage-item
  transition: all 0.5s

.plannerpage-item-enter, .plannerpage-item-leave-to
  opacity: 0
  transform: scale(0)

.plannerpage-item-enter-to
  opacity: 1
  transform: scale(1)

.plannerpage-item-move
