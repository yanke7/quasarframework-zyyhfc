<template>
  <q-page padding>
    <div v-if="$q.platform.is.mobile">
      If on mobile, try rotating your screen to landscape mode for best viewing
      of the Planner<br /><br />
    </div>

    <q-markdown>
      This page is a **WIP** - please consider making a **PR** to make this
      better, it will be appreciated :heart:. You can view the source for
      [Planner](https://github.com/quasarframework/quasar-ui-qcalendar/blob/dev/demo/src/pages/Planner.vue)
      and
      [PlannerItem](https://github.com/quasarframework/quasar-ui-qcalendar/blob/dev/demo/src/components/PlannerItem.vue)
      on Github. Drag-and-Drop has been implemented. Give it a try. :) ---
    </q-markdown>
    ...
    >
        ...
        <template v-if="id === 'over-due'">
          <div class="row items-center no-wrap">
            <q-icon
              ...
              :class="
                'cursor-pointer' +
                (overdueSelected ? ' text-red-8' : ' text-blue-8')
              "
      ...
      <template #day-header-label="{ timestamp }">
        <div class="row items-center no-wrap">
          <q-icon
            :name="
              selected[timestamp.weekday - 1] === true
                ? 'check_box'
                : 'check_box_outline_blank'
            "
            :class="
              'cursor-pointer' +
              (selected[timestamp.weekday - 1] ? ' text-red-8' : ' text-blue-8')
            "
            @click.stop.prevent="
              $set(
                selected,
                timestamp.weekday - 1,
                !selected[timestamp.weekday - 1]
              )
            "
            style="font-size: 24px"
            v-ripple
          />
          <span class="ellipsis">{{
            weekdayFormatter(timestamp, $q.screen.lt.lg)
          }}</span>
        </div>
      </template>
      ...
      <template #column-body="{ column }">
        <template v-if="column.id === 'over-due'">
          ...
          <div
            class="planners-column"
            data-column="overdue"
            @dragover.stop="onDragOver"
            @drop.stop="onDrop"
          >
            <transition-group name="planners-item">
...
export default {
  ...
  components: {
    PlannersItem: () => import('../components/PlannersItem'),
  },
  ...
  methods: {
    ...
    generateList(list, count, timestamp, overdue = false) {
      ...
      for (let i = 0; i < count; ++i) {
        ...
        items[i].id = itemId++;
        items[i].address =
          addresses[Math.floor((Math.random() * 100) % addresses.length)];
        items[i].name = names[Math.floor((Math.random() * 100) % names.length)];
        items[i].email =
          emails[Math.floor((Math.random() * 100) % emails.length)];
        items[i].phone =
          phones[Math.floor((Math.random() * 100) % phones.length)];
        items[i].amount = this.generateAmount();
        items[i].workDate =
          overdue === true ? this.generateDate(timestamp) : timestamp.date;
        items[i].workDone =
          workDone[Math.floor((Math.random() * 100) % workDone.length)];
        items[i].daysOver =
          overdue === true
            ? this.getDaysBetween(items[i].workDate, this.today)
            : 0;
      }
    ...
    generateDate(startTimestamp) {
      ...
      let ts = QCalendar.moveRelativeDays(
        startTimestamp,
        QCalendar.prevDay,
        days
      );
    ...
    generateAmount() {
      ...
      const fractional = QCalendar.padNumber(
    ...
    onDragStart(e, item) {
      ...
      const child = this.getCorrectTarget(e.currentTarget, 'Planners-item');
    ...
    onDragEnter(e, column, child) {
      if (!column && !child) {
        column = this.getCorrectTarget(e.currentTarget, 'planners-column');
        child = this.getCorrectTarget(e.currentTarget, 'planners-item');
      }
    ...
    onDrop(e, column, child) {
      ...
      if (targetColumn) {
        ...
        const currentColumnEl = this.getCorrectTarget(
          this.dragEl,
          'planners-column'
        );
        if (currentColumnEl) {
    ...
    findTargets() {
      ...
      for (let i = 0; i < els.length; ++i) {
        const el = els[i];
        if (el.classList.contains('planners-item')) {
          child = el;
        } else if (el.classList.contains('planners-column')) {
          column = el;
        }
    ...
    onTouchMove(e, item) {
      ...
      if (column || child) {
        ...
        const { left, top } = this.getTouchOffsets(
          this.copyElement,
          this.pageX,
          this.pageY
        );
...
.planners-column.drag-over
  border: 1px dashed red

.planners-item.drag-over-item
  background: rgba(0,255,0, .1)

.planners-item
  transition: all 0.5s

.planners-item-enter, .planners-item-leave-to
  opacity: 0
  transform: scale(0)

.planners-item-enter-to
  opacity: 1
  transform: scale(1)
